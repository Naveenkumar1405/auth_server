...
class OauthServer {
    ...
    createToken(name, email, expires_in, client_state) {
        ...
        const access_token = "ACCT-" + randomstring.generate(6);
        const refresh_token = "REFT-" + randomstring.generate(6);
        ...
        this.tokenIdData[id_token] = this.authHeaderData["Bearer " + access_token] = {
            email: email,
            email_verified: true,
            name: name
        };
        this.code[code] = token;
        this.refreshData[refresh_token] = {
            name: name,
            email: email,
            expires_in: expires_in
        };
